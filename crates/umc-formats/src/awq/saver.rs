...
impl FormatSaver for AwqSaver {
        ...
        -> Result<(), UmcError>
    {
        ...
        ir_clone.metadata.insert("__format__", umc_core::ir::MetaValue::String("AWQ".into()));
        ir_clone.metadata.insert("quant_type", umc_core::ir::MetaValue::String("awq".into()));
        if let Some(bits) = ir.metadata.get_i64("quantization.bits") {
            ir_clone.metadata.insert("w_bit", umc_core::ir::MetaValue::I64(bits));
        }
        if let Some(gs) = ir.metadata.get_i64("quantization.group_size") {
            ir_clone.metadata.insert("q_group_size", umc_core::ir::MetaValue::I64(gs));
        }
