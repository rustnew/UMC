...
impl FormatSaver for GptqSaver {
        ...
        -> Result<(), UmcError>
    {
        ...
        ir_clone.metadata.insert("__format__", umc_core::ir::MetaValue::String("GPTQ".into()));
        ir_clone.metadata.insert("quant_method", umc_core::ir::MetaValue::String("gptq".into()));
        if let Some(bits) = ir.metadata.get_i64("quantization.bits") {
            ir_clone.metadata.insert("bits", umc_core::ir::MetaValue::I64(bits));
        }
        if let Some(gs) = ir.metadata.get_i64("quantization.group_size") {
            ir_clone.metadata.insert("group_size", umc_core::ir::MetaValue::I64(gs));
        }
