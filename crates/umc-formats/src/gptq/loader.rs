...
impl FormatLoader for GptqLoader {
        ...
        -> Result<UniversalIR, UmcError>
    {
        ...
        ir.metadata.insert("source_format", MetaValue::String("GPTQ".into()));
        ir.metadata.insert("quantization.method", MetaValue::String("gptq".into()));
        ...
        ir.quantization = Some(QuantizationStore {
            scheme: QuantScheme::Gptq { bits: bits as u8, sym: false },
...
mod tests {
    ...
    fn make_gptq_ir() -> UniversalIR {
        ...
        ir.tensors.insert(Tensor::from_bytes("layer.qweight", DType::I32, vec![4, 4], qweight)).unwrap();
        ir.tensors.insert(Tensor::from_bytes("layer.scales", DType::F16, vec![1, 4], scales)).unwrap();
        ir.tensors.insert(Tensor::from_bytes("layer.qzeros", DType::I32, vec![1, 1], qzeros)).unwrap();
        ir.metadata.insert("quantization.method", MetaValue::String("gptq".into()));
        ir.metadata.insert("quantization.bits", MetaValue::I64(4));
        ir
    }
    ...
    fn test_gptq_round_trip() {
        ...
        let st_loader = crate::safetensors::SafeTensorsLoader;
        let loaded = st_loader.load(tmp.path(), &LoadOptions::default(), &ProgressCallback::noop()).unwrap();
