...
use umc_core::{AdapterInfo, AdapterType, UmcError, UniversalIR, UMC_VERSION};
    ...
    fn load(
        ...
        let adapter_path = if path.is_dir() {
            ...
            candidates
                ...
                .find(|p| p.exists())
                .ok_or_else(|| {
                    UmcError::Other("LoRA: no adapter_model.safetensors found in directory".into())
                })?
        } else {
            path.to_path_buf()
        };
        ...
        ir.metadata
            .insert("source_format", MetaValue::String("LoRA".into()));
        if let Some(r) = rank {
            ir.metadata.insert("lora.rank", MetaValue::I64(r as i64));
        }
        if let Some(a) = alpha {
            ir.metadata.insert("lora.alpha", MetaValue::F64(a));
        }
