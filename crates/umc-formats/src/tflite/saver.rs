...
impl FormatSaver for TFLiteSaver {
    ...
    fn default_extension(&self) -> &'static str { "tflite" }
...
struct Fb { buf: Vec<u8> }

impl Fb {
    fn new() -> Self { Self { buf: Vec::new() } }
    fn pos(&self) -> usize { self.buf.len() }

    fn pu16(&mut self, v: u16) { self.buf.extend_from_slice(&v.to_le_bytes()); }
    fn pu32(&mut self, v: u32) { self.buf.extend_from_slice(&v.to_le_bytes()); }
    fn pi32(&mut self, v: i32) { self.buf.extend_from_slice(&v.to_le_bytes()); }

    fn align4(&mut self) { while self.buf.len() % 4 != 0 { self.buf.push(0); } }
    fn align2(&mut self) { while self.buf.len() % 2 != 0 { self.buf.push(0); } }

    /// Reserve a u32 slot (placeholder 0) and return its position.
    fn slot(&mut self) -> usize { let p = self.pos(); self.pu32(0); p }

    /// Patch a slot with a FORWARD offset: target_pos - slot_pos.
    fn patch(&mut self, slot_pos: usize, target_pos: usize) {
        debug_assert!(target_pos >= slot_pos, "patch: backward offset {}->{}", slot_pos, target_pos);
    ...
    fn write_vtable(&mut self, field_offsets: &[u16]) -> usize {
        ...
        for &fo in field_offsets { self.pu16(fo); }
        vt_pos
    }
...
fn build_tflite_flatbuffer(ir: &UniversalIR, progress: &ProgressCallback) -> Result<Vec<u8>, UmcError> {
