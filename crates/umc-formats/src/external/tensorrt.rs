...
impl FormatSaver for TensorRTSaver {
    fn format_name(&self) -> &'static str { "TensorRT" }
    fn default_extension(&self) -> &'static str { "engine" }
        ...
        -> Result<(), UmcError>
    {
        ...
        let precision = ir.metadata.get_str("tensorrt.precision")
            .unwrap_or("fp32");
        let mut args = vec![
        ...
        match precision {
            ...
            "int8" => { args.push("--fp16".to_string()); args.push("--int8".to_string()); }
            _ => {}
        }
...
mod tests {
    ...
    fn test_tensorrt_graceful_missing_tool() {
        ...
        match result {
            Ok(_) => {} // installed, fine
            Err(e) => assert!(e.to_string().contains("not found") || e.to_string().contains("failed")),
        }
    }
}
