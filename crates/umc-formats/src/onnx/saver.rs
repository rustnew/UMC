...
impl FormatSaver for OnnxSaver {
    ...
    fn default_extension(&self) -> &'static str { "onnx" }
    ...
    fn save(
        ...
        match result {
            Ok(()) => {
                std::fs::rename(&tmp, path).map_err(|e| UmcError::AtomicRename {
...
impl OnnxSaver {
    fn write_to_path(
                ...
                match umc_dtype_to_onnx(&tensor.dtype) {
                    Some(dt) => {
                        let raw = tensor.data.as_bytes().map_err(|e| {
                            UmcError::Other(format!("Tensor '{}': {}", name, e))
                        })?;
                    ...
                    None => {
                        ...
                        let f32_bytes = dequantize_any_to_f32_bytes(raw, &tensor.dtype, n_elems)
                            .map_err(|e| UmcError::Other(
                                format!("ONNX dequant tensor '{}' ({:?}): {}", name, tensor.dtype, e)
                            ))?;
                        let onnx_f32 = umc_dtype_to_onnx(&DType::F32).unwrap();
                        progress.report(&format!("Dequantizing '{}' ({:?})→F32", name, tensor.dtype));
        ...
        model.encode(&mut buf).map_err(|e| {
            UmcError::Other(format!("ONNX encode error: {}", e))
        })?;
...
mod tests {
    use super::*;
    use umc_core::{DType, Tensor, UniversalIR, FormatLoader, LoadOptions};
    ...
    fn make_f32_ir() -> UniversalIR {
        ...
        let data: Vec<u8> = [1.0f32, 2.0, 3.0, 4.0].iter()
            .flat_map(|v: &f32| v.to_le_bytes()).collect();
        ir.tensors.insert(Tensor::from_bytes("weight", DType::F32, vec![2, 2], data)).unwrap();
        ir
    }
    ...
    fn test_save_creates_valid_onnx() {
        ...
        OnnxSaver.save(&ir, f.path(), &SaveOptions::default(), &ProgressCallback::noop()).unwrap();
    ...
    fn test_round_trip_f32() {
        ...
        let ir2 = OnnxLoader.load(f.path(), &LoadOptions::default(), &ProgressCallback::noop()).unwrap();
        ...
        let loaded: Vec<f32> = t.data.as_bytes().unwrap().chunks(4)
            ...
            .collect();
        assert_eq!(loaded, original, "F32 ONNX round-trip must be bit-identical");
    }
}
