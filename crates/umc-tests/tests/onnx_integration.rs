...
fn make_onnx_bytes_f32(tensors: &[(&str, Vec<usize>, Vec<f32>)]) -> Vec<u8> {
    ...
    let initializers: Vec<_> = tensors
        .iter()
...
fn test_onnx_round_trip_multiple_tensors() {
    ...
    let refs: Vec<(&str, Vec<usize>, Vec<f32>)> = tensors_data
        .iter()
...
fn test_onnx_metadata_preserved() {
    ...
    let ir = OnnxLoader
        ...
        .unwrap();
    assert!(
        ir.metadata.get("onnx.ir_version").is_some(),
        "ir_version missing"
    );
    assert!(
        ir.metadata.get("onnx.opset_version").is_some(),
        "opset_version missing"
    );
