...
fn test_gguf_load_empty_model() {
    ...
    let ir = GgufLoader.load(f.path(), &LoadOptions::default(), &ProgressCallback::noop()).unwrap();
...
fn test_gguf_load_single_f32_tensor() {
    ...
    let loaded: Vec<f32> = bytes.chunks(4)
...
fn test_gguf_provenance_chain_valid() {
    let f = write_gguf_v3_with_f32_tensors(&[
        ("layer.0.weight", vec![4, 4], vec![0.0f32; 16]),
    ]);
    let ir = GgufLoader.load(f.path(), &LoadOptions::default(), &ProgressCallback::noop()).unwrap();
    assert!(ir.provenance.verify(), "Provenance chain must pass integrity check");
...
fn test_gguf_metadata_only_loads_zero_tensors() {
    ...
    let ir = GgufLoader.load(f.path(), &opts, &ProgressCallback::noop()).unwrap();
...
fn test_gguf_v1_v2_v3_all_accepted() {
    for version in [1u32, 2, 3] {
        ...
        assert!(result.is_ok(), "GGUF v{} should be accepted: {:?}", version, result);
    }
}
